...
describe('<KeyValueEditor />', () => {
	it('Should be able to render with data array', () => {
		const keyValueEditorComponent = mount(<KeyValueEditor data={DATA_OBJ_MOCK} />);

		const inputElement = keyValueEditorComponent.find('input');

		expect(inputElement.length).toEqual(Object.keys(DATA_OBJ_MOCK).length);
	});
	...
	it('Should be able to render with data object', () => {
		...
		expect(inputElement.length).toEqual(DATA_ARRAY_MOCK.length);
	});

	it('Should set the correct className', () => {
		const keyValueEditorComponent = shallow(
			<KeyValueEditor data={DATA_OBJ_MOCK} className={customClass} />
		);
	...
	it('Should be able to set the readonly state', () => {
		const keyValueEditorComponent = mount(<KeyValueEditor data={DATA_OBJ_MOCK} readonly />);
		...
		expect(spanElements.length).toEqual(Object.keys(DATA_OBJ_MOCK).length * 2); // labels and values
	});
	...
	it('Should call the onChange handler when an input triggers an onBlur event', () => {
		...
		const keyValueEditorComponent = mount(
			<KeyValueEditor data={DATA_OBJ_MOCK} onChange={onChange} />
		);
